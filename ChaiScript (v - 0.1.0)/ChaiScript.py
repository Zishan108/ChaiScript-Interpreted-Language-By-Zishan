  ...
  def make_tokens(self):
    ...
    while self.current_char != None:
      if self.current_char in ' \t':
        ...
        return [], IllegalCharError(pos_start, self.pos, "'.'")  # Only if it's not part of a valid identifier
      else:
        pos_start = self.pos.copy()
...
class Parser:
  ...
  def atom(self):
    ...
    if tok.type in (TT_INT, TT_FLOAT):
        ...
        res.register_advancement()
        self.advance()
        var_access_node = VarAccessNode(tok)

        # Check for indexing after the identifier
        if self.current_tok.type == TT_LSQUARE:
            ...
            res.register_advancement()
            self.advance()
            # Add pos_start and pos_end to IndexAccessNode
            return res.success(IndexAccessNode(var_access_node, index_expr, tok.pos_start, self.current_tok.pos_end))

        # If no indexing, return the variable access node
        return res.success(var_access_node)
...
class Interpreter:
  ...
  def visit_IndexAccessNode(self, node, context):
    list_value = self.visit(node.list_node, context)
    index_value = self.visit(node.index_node, context)

    if not isinstance(list_value, list):
        return RTResult().failure(RTError(
            node.pos_start, node.pos_end,
            "Trying to index a non-list value",
            context
        ))
    ...
    return RTResult().success(list_value[index_value])
